...
public class XMLSchemaLoader implements SchemaLoader {
	...
	private final Map<String, TableCryptoConfig> tableCrytos;
	private final ConcurrentMap<String, CrytoAlgorithm> crytoCacheMap = new ConcurrentHashMap<String, CrytoAlgorithm>();

	public XMLSchemaLoader(String schemaFile, String ruleFile, String cryptoFile) {
		XMLRuleLoader ruleLoader = new XMLRuleLoader(ruleFile);
		XMLCryptoLoader xmlCryptoLoader = new XMLCryptoLoader(cryptoFile);
		this.tableRules = ruleLoader.getTableRules();
		this.tableCrytos = xmlCryptoLoader.getTableCryptos();
		ruleLoader = null;
	...
	private void loadSchemas(Element root) {
		...
		for (int i = 0, n = list.getLength(); i < n; i++) {
			...
			// wcl--start 加入到缓存中
			/*
			 * ConcurrentMap<String, CrytoAlgorithm> crytoCacheMap = MycatServer
			 * .getInstance().getCrytoCacheMap();
			 */
			for (Map.Entry<String, TableConfig> entry : tables.entrySet()) {
				CryptoConfig[] cryptoConfigArr = entry.getValue().getCrypto();
				if (cryptoConfigArr != null) {
					for (int j = 0; j < cryptoConfigArr.length; j++) {
						CryptoConfig cryptoConfig = cryptoConfigArr[j];
						String column = cryptoConfig.getColumns();
						String table = entry.getKey();
						String schema = name;
						String key = schema + "|" + table + "|" + column;
						crytoCacheMap.put(key.toLowerCase(),
								cryptoConfig.getCrytoAlgorithm());
					}
				}
			}
			// wcl--end
	 ...
	 * @return
	 */
	private String doTableNameSuffix(String tableNameElement,
			String tableNameSuffixElement) {
	...
	private Map<String, TableConfig> loadTables(Element node) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			// TODO:路由, 增加对动态日期表的支持
			String tableNameSuffixElement = tableElement.getAttribute(
			...
			if (!"".equals(tableNameSuffixElement)) {
				...
				if (tableNameElement.split(",").length > 1) {
					throw new ConfigException(
							"nameSuffix "
									+ tableNameSuffixElement
									+ ", require name parameter cannot multiple breaks!");
				}

				tableNameElement = doTableNameSuffix(tableNameElement,
						tableNameSuffixElement);
			}
			...
			// ----wcl-start
			TableCryptoConfig tableCryptoConfig = null;
			if (tableElement.hasAttribute("crypto")) {
				String cryptoName = tableElement.getAttribute("crypto");
				tableCryptoConfig = tableCrytos.get(cryptoName);
				if (tableRule == null) {
					throw new ConfigException("crypto" + cryptoName
							+ " is not found!");
				}
			}
			// ----wcl-end
			...
			for (int j = 0; j < tableNames.length; j++) {
				...
				if (tableCryptoConfig != null) {
					table.setCrypto(tableCryptoConfig.getCrypto());
				}
	...
	private DBHostConfig createDBHostConf(String dataHost, Element node,
		...
		int weight = "".equals(weightStr) ? PhysicalDBPool.WEIGHT : Integer
				.valueOf(weightStr);
	...
	private void loadDataHosts(Element root) {
		...
		for (int i = 0, n = list.getLength(); i < n; ++i) {
			...
			String tempReadHostAvailableStr = element
					.getAttribute("tempReadHostAvailable");
			boolean tempReadHostAvailable = tempReadHostAvailableStr.equals("") ? false
					: Integer.valueOf(tempReadHostAvailableStr) > 0;
			...
			long logTime = "".equals(logTimeStr) ? PhysicalDBPool.LONG_TIME
					: Long.valueOf(logTimeStr);
				...
				if (readNodes.getLength() != 0) {
					...
					for (int r = 0; r < readDbConfs.length; r++) {
						...
						readDbConfs[r] = createDBHostConf(name, readNode,
								dbType, dbDriver, maxCon, minCon, filters,
								logTime);
					}
	...
	@Override
	public ConcurrentMap<String, CrytoAlgorithm> getTableCryptos() {
		// TODO Auto-generated method stub
		return crytoCacheMap;
	}
