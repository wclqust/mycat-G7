...
public class DruidMycatRouteStrategy extends AbstractRouteStrategy {
	...
	public RouteResultset routeNormalSqlWithAST(SchemaConfig schema,
		...
		if (schema.isNeedSupportMultiDBType()) {//这是判断是否是支持多种类型的数据库  不能表示本次语句的sql是操作的mysql数据库
		 ...
		 */
		try {
			statement = parser.parseStatement();//解析出是什么类型的sql语句
		 ...
		 */
		checkUnSupportedStatement(statement);//屏蔽某些sql语句


		DruidParser druidParser = DruidParserFactory.create(schema, statement, visitor);//此处解析出来的prase有可能是针对某些特殊类型的数据库 比如oracle sqlserevr 等
