...
public class SingleNodeHandler implements ResponseHandler, Terminatable,
	...
	private String[] fieldNameAndTableNameArr = null;
	...
	public SingleNodeHandler(RouteResultset rrs, NonBlockingSession session) {
		...
		if (schema != null && ServerParse.SHOW == rrs.getSqlType()) {
			SchemaConfig schemaConfig = MycatServer.getInstance().getConfig()
					.getSchemas().get(schema);
			int type = ServerParseShow.tableCheck(rrs.getStatement(), 0);
			isDefaultNodeShowTable = (ServerParseShow.TABLES == type && !Strings
					.isNullOrEmpty(schemaConfig.getDataNode()));
			isDefaultNodeShowFullTable = (ServerParseShow.FULLTABLES == type && !Strings
					.isNullOrEmpty(schemaConfig.getDataNode()));

			if (isDefaultNodeShowTable) {
				shardingTablesSet = ShowTables.getTableSet(source,
						rrs.getStatement());
			} else if (isDefaultNodeShowFullTable) {
				shardingTablesSet = ShowFullTables.getTableSet(source,
						rrs.getStatement());
			}
		}
	}
	...
	public void fieldEofResponse(byte[] header, List<byte[]> fields,
		...
		fieldNameAndTableNameArr = new String[fieldCount];

		// TODO: add by zhuam
		// 查询结果派发
		QueryResult queryResult = new QueryResult(
				session.getSource().getUser(), rrs.getSqlType(),
				rrs.getStatement(), startTime, System.currentTimeMillis());
		QueryResultDispatcher.dispatchQuery(queryResult);
		...
		for (int i = 0, len = fields.size(); i < len; ++i) {
			...
			fieldPkg.read(field);
			fieldNameAndTableNameArr[i] = new String(fieldPkg.orgName)
					.toLowerCase()
					+ "|"
					+ new String(fieldPkg.table).toLowerCase();
		}
		eof[3] = ++packetId;
		...
		if (isDefaultNodeShowTable) {
			for (String name : shardingTablesSet) {
				...
				row.add(StringUtil.encode(name.toLowerCase(),
						source.getCharset()));
	...
	public void rowResponse(byte[] row, BackendConnection conn) {
		...
		List<byte[]> fieldValues = rowDataPkg.fieldValues;
		/*
		 * System.out.println("------------------获取到 各列的值--------------------" +
		 * fieldNameAndTableNameArr.toString());
		 */
		...
		/*
		 * System.out.println("------------------修改获取到 各列的值--------------------")
		 * ;
		 */
		ByteBuffer byteBuf = session.getSource().allocate();
		byteBuf = rowDataPkg.write(byteBuf, session.getSource(), false);
		byteBuf.flip();
		row = new byte[byteBuf.limit()];
		byteBuf.get(row);
		byteBuf.clear();
		/*********** 解密結束 *******************/
