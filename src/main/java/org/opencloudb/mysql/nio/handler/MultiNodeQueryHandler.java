...
public class MultiNodeQueryHandler extends MultiNodeHandler implements
		...
		try {
			...
			for (int i = 0, len = fieldCount; i < len; ++i) {
				...
				if (needMerg) {
					...
					/*
					 * fieldNameAndTableNameArr[i] = new
					 * String(fieldPkg.orgName) .toLowerCase() + "|" + new
					 * String(fieldPkg.table).toLowerCase();
					 */
					if (columToIndx != null
	...
	@Override
	public void rowResponse(byte[] row, final BackendConnection conn) {
		if (errorRepsponsed.get()) {
			conn.close(error);
			return;
		}
		...
		try {
			...
			RowDataPacket rowDataPkg = new RowDataPacket(fieldCount);// fieldCount查询列数
			rowDataPkg.read(row);
